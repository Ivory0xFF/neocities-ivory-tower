window.onload = function () {
    ...
    for (i = 0; i < p.length; i++) {
        ...
        for (a = 0; a < colors.length; a++) {
            ...
            // div.addEventListener("onclick",navigator.clipboard.writeText(colors[a]));
            // div.addEventListener("mouseente  r",()=>{div.innerText=`${window.getComputedStyle(div).backgroundColor}`});
            // div.addEventListener("mouseleave",()=>{div.innerText=""});
        }
    }
};